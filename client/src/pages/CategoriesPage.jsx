...
<div className='lg:px-[7rem] md:px-[5rem] px-5 bg-slate-100 min-h-screen pt-28'>
  ...
  <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  gap-6'>
    ...
    <div key={index} className='h-40 bg-gray-300 rounded-lg animate-pulse'></div>
    ))}
  </div>
  ) : (
  <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
  ...
  <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-50'>
    <div className='bg-white rounded-lg p-6 w-96'>
      ...
      <input type='text' placeholder='Category Name' value={newCategory.name} onChange={(e)=>
      setNewCategory((prev) => ({ ...prev, name: e.target.value }))
      }
      className='border rounded p-2 w-full mb-4'
      />
      <select value={newCategory.type} onChange={(e)=>
      ...
      <div className='flex justify-end'>
        ...
        <button onClick={handleAddCategory} className='px-4 py-2 bg-primaryColor text-white rounded'>
          Add Category
        </button>
      </div>
    </div>
